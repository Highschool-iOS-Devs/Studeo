...
struct RegistrationView: View {
    ...
    @State private var email: String = "j"
    ...
    var body: some View {
        ZStack {
            ...
            .disabled(showLoadingAnimation ? true : false)
    ...
    func sendData(performActions: @escaping (ErrorModel, User?) -> Void) {
        Auth.auth().createUser(withEmail: self.email, password: self.password) { authResult, error in
            ...
            self.userData.userID = newUser.id.uuidString
            self.userData.name = newUser.name
                do {
        ...
        func saveDefaults(){
            ...
            if let currentUser = Auth.auth().currentUser{
                do{
                    try db.collection("userSettings").document(currentUser.uid).setData(from: userData)
                }
                catch{
