...
struct RegistrationView: View {
    ...
    func sendData(performActions: @escaping (ErrorModel, User?) -> Void) {
        Auth.auth().createUser(withEmail: self.email, password: self.password) { authResult, error in
            ...
            let newUser = User(id: UUID(), firebaseID: authResult!.user.uid, name: self.username, email: self.email, isMentor: false, studyHours: [0], studyDate: ["9-16-2020"], all: 0, month: 0, day: 0, description: "Tap here to create your bio", isAvailable: true, finishedOnboarding: false)
